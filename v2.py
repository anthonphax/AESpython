...
def expand_key(key):
    expanded_key = np.zeros((44, 4), dtype=np.uint8)  # 44 para AES-128
    return expanded_key.flatten()

def aes_encrypt(plaintext, key):
    state = np.frombuffer(hex_string_to_bytes(plaintext), dtype=np.uint8)
    key = np.frombuffer(hex_string_to_bytes(key), dtype=np.uint8)
